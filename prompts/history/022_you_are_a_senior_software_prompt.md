# Executed: 2026-04-05T13:15:09.269875
...
1. **Check `.gitignore`**: Make sure the database configuration file (`.db-config`) is listed in `.gitignore` so it's never committed.

2. **Write an API server spec**: Create a new spec document in `docs/specs/` for the Gem Factory API server. Base it on the architecture defined in `docs/context/ARCH.md` — pull the relevant backend API details (endpoints, data flows, authentication, Cloud Run deployment, database interactions) from that document and expand them into a full specification.
