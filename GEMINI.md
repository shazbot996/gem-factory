...
- `GET_ALL_GEMS` — returns the current local pending cache.
- `DELETE_GEM` — remove a gem from the local pending cache (called by the
  popup after a successful upload, or after auto-reconcile finds a stale
  local copy of an already-in-cloud gem).
...
- The Chrome extension lives in `extension/` and writes to GCS using
  ...
  manifest).
- The SPA is an admin-only list/view/delete tool against GCS; there is
  no API server to start, no database to seed. `make spa-dev` is all you
  need for the frontend.
